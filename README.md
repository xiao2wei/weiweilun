...
面向两阶段边缘决策的 `default`、`burst`、`compute_pressure`、`capacity_pressure` 矩阵，pilot gate、正式统计 family 与审计顺序见 [docs/PAPER_V1_EXPERIMENTS.md](docs/PAPER_V1_EXPERIMENTS.md)。该协议把到达压力、本地计算压力和 RSU admission 压力分别成对控制，并使用不同的 pilot/formal/sensitivity seeds。
...
默认不允许覆盖任何目标 leaf 或批处理索引。`run-all`、`multi-seed` 和 `sweep` 会在启动第一项之前预检整个批次。sweep 开始时写入 `sweep.in_progress.json`，结束后原子写索引并删除标记；聚合器拒绝带中断标记的结果树。只有明确希望重算同一目录时使用 `--overwrite`；该选项只清理本工具已知布局，不应把结果目录与其他资料混用。

正式 `run-numerical-study` 和 `sweep` 还要求可执行包源码对应当前 Git commit。study 必须绑定 `paper-v1-load-matrix.json` 中的 scale/regime/analysis family；正式 sweep 必须绑定 `paper-v1-sensitivity.json` 的 factor 和主矩阵参考。批处理开始时会冻结 base config/profile/trace/evidence 的精确身份，并在每个 environment/case 前后和发布索引前复核；敏感性分析还会跨全部环境核对 profile、evidence 与固定 scenario trace，拒绝运行期间混入不同数值总体。建议先提交已验证代码并显式检查：

```powershell
python -m privacy_edge_sim.cli source-preflight
```

`--allow-dirty-source` 仅供开发试跑；使用该开关的结果不得并入正式统计。
...
上述 `multi-seed` 保留为“同一冻结环境下的控制/采样随机流”实验。真正的多环境数值研究使用同一 profile、攻击证据和 scenario 分布，为每个 environment seed 生成独立 evaluation trace，并在相同环境上成对运行全部策略。下面是明确标记为 development 的最小试跑；正式命令使用实验手册中的完整注册参数：

```powershell
python -m privacy_edge_sim.cli run-numerical-study `
  --base-study-root studies/base `
  --environment-seeds 201,202,203,204,205 `
  --policies all_local,safe_lyapunov_h1 `
  --baseline all_local `
  --output-root results/numerical-study-pilot `
  --allow-dirty-source
```

入口会输出每个环境/策略的 manifest、严格配对统计记录、机制诊断和 `study_report.json`。统计单位是 environment，不把同一 trace 内任务伪装成独立 seed；报告策略减基线的配对差、Cohen’s dz、环境 cluster bootstrap 95% CI、双侧 sign-flip p 值及 Holm 校正。Paper-v1 正式矩阵冻结为 30 个环境 seed（301–330）；先用独立 pilot seeds 201–205 验证机制，再运行正式批次。

参数扫描使用点路径；数组下标也作为路径段。示例文件见 `examples/sweep-small.json`：

```powershell
python -m privacy_edge_sim.cli sweep `
  --config studies/base/configs/numerical_default.json `
  --policy esl_smpc `
...
`sweep_diagnostics.json` 会报告配置变化是否真的改变 edge 路径、队列、损失或能耗。每个坐标必须至少包含两个不同 level；完全相同或物理机制未激活的扫描会明确告警，而不是被当作参数敏感性证据。正式 OFAT 包含隐私策略边界、Lyapunov 权重、MPC horizon、场景数和 RSU admission 容量五个登记因素，使用 401–420 独立环境；`sweep-rsu-admission-concurrency.json` 使用 `$paired` 锁步改变两台 RSU，不会产生错误的笛卡尔积。正式 sweep 还会拒绝未登记 factor、错误参考 regime 和不完整 case/index 树。

完成 401–420 全部登记 sweep 后，用环境级配对入口生成探索性敏感性报告：

```powershell
python -m privacy_edge_sim.cli analyze-sensitivity `
...
该入口拒绝缺 seed、缺 level、重复 factor/seed、trace 未配对、manifest/summary 哈希
错误和非 clean-source 结果；同一环境内各 level 不会被误当作独立重复。每个 factor
只做其内部的 level-vs-reference Holm 校正，factor 之间保持探索性分离。
...
跨 run 的失败成本覆盖审计会递归发现 manifest，并验证 manifest 自哈希、clean committed source、零失败不变量，以及 `tasks.csv`、`actions.jsonl`、`events.jsonl` 的绑定校验和：

```powershell
python -m privacy_edge_sim.cli audit-failure-coverage `
  ...
  --output results/paper-v1-pilot-audits/failure-coverage.json
```

未出现的类别保持 `NOT_OBSERVED`；不会因为其他成本字段存在就被升级为已验证。`--allow-unverified-inputs` 只用于开发取证，并在报告中标记为未验证。
...
- `manifest.json`：Git/source cleanliness、原始与 LF-portable 源码 hash、位置无关的语义配置 hash、运行时配置 hash、版本、trace 校验和、数据标签、来源、资源、网络、负载、deadline、控制器、独立种子、软件环境、仿真时间和不变量结果；

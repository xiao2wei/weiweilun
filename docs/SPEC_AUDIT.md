...
当前数值 evidence/profile 版本为 `numerical-study-2.1.0` / `2.2.0`，trace 版本为 `2.3.0`；12 条管线及三个攻击器使用 `numerical_v2` 标识。它不是对方案核心方法的替换，而是“没有真实硬件/攻击模型时，以数值仿真闭合论文实验”的离线数据生成实现。Paper-v1 的显式到达窗口和可控预处理故障仅改变冻结数值场景，不会把数值仿真表述为硬件或真实攻击测量。
